...
async function assertSuper(userId: string) {
  ...
  const { data } = await supabaseAdmin.from("user_roles" as any).select("role").eq("user_id", userId).eq("role", "SUPERADMIN");
...
export const seedDemoData = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context }) => {
    ...
    // 1. Foundation
    const { data: f, error: fe } = await supabaseAdmin.from("foundations" as any)
    ...
    // 2. School
    const { data: s, error: se } = await supabaseAdmin.from("schools" as any)
    ...
    // 3. Academic Year
    const { data: ay, error: aye } = await supabaseAdmin.from("academic_years" as any)
    ...
    // 4. Terms
    await supabaseAdmin.from("academic_terms" as any).upsert([
    ...
    await supabaseAdmin.from("staff" as any).upsert(
      teachers.map((t) => ({ ...t, school_id: s.id, employment_type: "TETAP_YAYASAN" as const, status: "ACTIVE" as const })),
      { onConflict: "school_id,nip" },
    );

    const { data: staffRows } = await supabaseAdmin.from("staff" as any).select("id, full_name, nip").eq("school_id", s.id);
    ...
    await supabaseAdmin.from("classes" as any).upsert(
      classes.map((c) => ({ ...c, school_id: s.id, academic_year_id: ay.id })),
      { onConflict: "academic_year_id,name" },
    );
    const { data: classRows } = await supabaseAdmin.from("classes" as any).select("id, name, grade_level").eq("academic_year_id", ay.id);

    // 8. Students (10 per class)
    const { data: existing } = await supabaseAdmin.from("students" as any).select("id").eq("school_id", s.id).limit(1);
    ...
    if (!existing?.length) {
      for (const cls of classRows ?? []) {
        ...
        const { data: inserted, error } = await supabaseAdmin.from("students" as any).insert(rows).select("id");
        ...
        await supabaseAdmin.from("student_enrollments" as any).insert(enr);
        // parents
        ...
        await supabaseAdmin.from("student_parents" as any).insert(parents);
      }
