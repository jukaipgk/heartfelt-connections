...
async function assertSuper(userId: string) {
  ...
  const { data } = await (supabaseAdmin as any).from("user_roles").select("role").eq("user_id", userId).eq("role", "SUPERADMIN");
...
export const seedDemoData = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context }) => {
    ...
    // 1. Foundation
    const { data: f, error: fe } = await (supabaseAdmin as any).from("foundations")
    ...
    // 2. School
    const { data: s, error: se } = await (supabaseAdmin as any).from("schools")
    ...
    // 3. Academic Year
    const { data: ay, error: aye } = await (supabaseAdmin as any).from("academic_years")
    ...
    // 4. Terms
    await (supabaseAdmin as any).from("academic_terms").upsert([
    ...
    await (supabaseAdmin as any).from("subjects").upsert(
    ...
    await (supabaseAdmin as any).from("staff").upsert(
      teachers.map((t) => ({ ...t, school_id: s.id, employment_type: "TETAP_YAYASAN" as const, status: "ACTIVE" as const })),
      { onConflict: "school_id,nip" },
    );

    const { data: staffRows } = await (supabaseAdmin as any).from("staff").select("id, full_name, nip").eq("school_id", s.id);
    ...
    await (supabaseAdmin as any).from("classes").upsert(
      classes.map((c) => ({ ...c, school_id: s.id, academic_year_id: ay.id })),
      { onConflict: "academic_year_id,name" },
    );
    const { data: classRows } = await (supabaseAdmin as any).from("classes").select("id, name, grade_level").eq("academic_year_id", ay.id);

    // 8. Students (10 per class)
    const { data: existing } = await (supabaseAdmin as any).from("students").select("id").eq("school_id", s.id).limit(1);
    ...
    if (!existing?.length) {
      for (const cls of classRows ?? []) {
        ...
        const { data: inserted, error } = await (supabaseAdmin as any).from("students").insert(rows).select("id");
        ...
        await (supabaseAdmin as any).from("student_enrollments").insert(enr);
        // parents
        ...
        await (supabaseAdmin as any).from("student_parents").insert(parents);
      }
